...
export default function ColorManager() {
  ...
  const fetchColors = async () => {
    ...
    if (fetchError) {
      ...
      setColors(data || []);
      // Fetch usage counts for each color
      const counts = {};
      for (const color of data || []) {
        const { count } = await countColorUsage(color.id);
        counts[color.id] = count || 0;
      }
  ...
  useUnsavedChanges(isDirty, t('admin.colors.unsaved', 'Des modifications ne sont pas sauvegardées. Quitter ?'));
  ...
  const handleSubmit = async e => {
    ...
    if (!payload.name) {
      pushToast({ message: t('admin.colors.error.nameRequired', 'Le nom est requis.'), variant: 'error' });
      return;
    }

    if (!payload.code) {
      pushToast({ message: t('admin.colors.error.codeRequired', 'Le code est requis.'), variant: 'error' });
      return;
    }

    if (!/^[a-z0-9_-]+$/.test(payload.code)) {
      pushToast({ message: t('admin.colors.error.codeInvalid', 'Code invalide.'), variant: 'error' });
      return;
    }

    if (!/^#([0-9A-F]{6})$/i.test(payload.hex_code)) {
      pushToast({ message: t('admin.colors.error.hexInvalid', 'Code couleur invalide (#RRGGBB).'), variant: 'error' });
      return;
    }
    ...
    if (saveError) {
      ...
      pushToast({ message: t('admin.colors.error.save', 'Sauvegarde impossible.'), variant: 'error' });
      return;
    }
  ...
  const handleDelete = async color => {
    ...
    if (usageError) {
      pushToast({ message: t('admin.colors.error.checkUsage', 'Impossible de vérifier l\'usage.'), variant: 'error' });
      return;
    }
    if (count && count > 0) {
      pushToast({
    ...
    if (deleteError) {
      pushToast({ message: t('admin.colors.error.delete', 'Suppression impossible.'), variant: 'error' });
      return;
    }
    pushToast({ message: t('admin.colors.success.delete', 'Couleur supprimée.'), variant: 'success' });
    fetchColors();
  };
    ...
    <div className="color-manager-v2">
      ...
      {filteredColors.length === 0 ? (
        <div className="empty-state">
          ...
          <p>{t('admin.colors.empty.description', 'Commencez par créer votre première couleur.')}</p>
          <button className="btn btn-primary" onClick={() => openModal()}>
            + {t('admin.colors.add', 'Nouvelle couleur')}
          </button>
        </div>
        ...
        <div className="color-palette-grid">
          {filteredColors.map(color => (
            <div
              ...
              style={{ '--color-hex': color.hex_code, '--color-contrast': getContrastColor(color.hex_code) }}
            >
              <div className="color-palette-card__swatch" style={{ backgroundColor: color.hex_code }}>
        ...
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-content modal-md" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3>{editingId ? t('admin.colors.editTitle', 'Modifier la couleur') : t('admin.colors.createTitle', 'Nouvelle couleur')}</h3>
            ...
            <form onSubmit={handleSubmit} className="modal-body">
                ...
                <div className="preset-colors-grid">
                  {PRESET_COLORS.map(preset => (
                    <button
                      key={preset.hex}
                      type="button"
                      className={`preset-color-btn ${currentHex === preset.hex ? 'is-selected' : ''}`}
              ...
              <div className="form-grid">
                ...
                <div className="form-group">
                  <label htmlFor="color-code">
                    {t('admin.colors.form.code', 'Code interne')} <span className="required">*</span>
                  </label>
                  <input
                    id="color-code"
                    name="code"
                    ...
                    required
                  />
                  <p className="input-hint">{t('admin.colors.form.codeHint', 'Slug technique (a-z, 0-9, tirets)')}</p>
                </div>

                <div className="form-group">
                  <label htmlFor="color-hex">
                    {t('admin.colors.form.hex', 'Code hexadécimal')} <span className="required">*</span>
                  </label>
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn btn-primary" disabled={saving}>
                  {saving ? t('admin.common.loading', 'Chargement...') : editingId ? t('admin.common.update', 'Mettre à jour') : t('admin.common.create', 'Créer')}
                </button>
              </div>
            </form>
          </div>
